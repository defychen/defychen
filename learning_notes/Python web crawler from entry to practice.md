...
[下载地址](http://ftp.ntu.edu.tw/MySQL/Downloads/MySQLInstaller/)
	...
	选择其中的"mysql-installer-community-5.7.24.0.msi"进行下载即可.
	PS:因为官网下载很慢.

**2.安装MySQL模块**

双击下载好的"mysql-installer-community-5.7.24.0.msi"文件--->Choosing a Setup Type中选择"Custom".在"Select Products and Features"中按下图选择:

![](images/mysql_install.png)

	PS:选择上右边的基本组件即可.

在下一步的"Check Requrements"可能出现部分组件没有安装,直接单击"Execute"会执行自动安装缺少的组件,安装完成后点击"Next"--->在Installation中点击"Execute"便开始安装MySQL模块.

**3.配置MySQL Server模块**

在之前的都是"Next"即可,在出现的"Type and Networking"中的Config Type选择"Development Computer"(个人电脑选择该项即可,不会占用太多的资源,也是默认选项)--->接下来就是输入密码,其他全部都"Next"即可.
...
1.命令行启动
	...
	1.启动
		"开始"菜单--->所有程序--->MySQL--->MySQL Server 8.0--->
		选择MySQL 8.0 Command Line Client-Unicode即可启动
	2.启动命令行之后会要求输入密码"Enter password"
		输入之前设置的密码
	3.在命令行测试命令的执行
		show databases ;
		/*
			此处一定要加分号";",且与databases之间要有一个空格.显示MySQL服务器中所有的数据库
			结果为:
...
2.界面启动
	...
	1.启动
		"开始"菜单--->所有程序--->MySQL--->选择"MySQL workbench 8.0 CE"--->在显示的界面中出现:
			"Welcome to MySQL Workbench",在下方有一个"MySQL Connections",点击下面的:
			"Local instance MySQL_DEFY ...",即可启动MySQL实例.也会要求输入密码.
	2.在出现的主窗口输入下面的代码
		show databases ;	//可以直接输入命令行进行相关的执行
	3.执行
		菜单执行:点击菜单中的"Query"--->选择"Execute Current Statement"
		快捷键执行:Ctrl+Enter
	4.之后则会显示当前MySQL服务器中的所有数据库
