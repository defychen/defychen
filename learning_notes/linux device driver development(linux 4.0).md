...
**字符设备驱动的框架:**
	...
	int xxx_open(struct inode *inode, struct file *filp)
	{
		...
		/*
		para1:设备结构体中成员的首地址.在probe的时候,调用"cdev_add(&dev->cdev, devno, 1);",将cdev加入到
			cdev链表中,同时设备号与cdev建立起了对应关系.因为设备文件与设备号一一对应,而每个设备文件都对应一
		...
		/*设备结构体变量作为文件(struct file *filp)的私有数据(private_data)，后续的read/write/ioctl/llseek
			函数通过filp->private_data访问设备结构体变量,体现linux面向对象的设计思想.
		PS:现在的做法是:
...
CFI(Common Flash Interface):公共闪存接口,是一个从NOR Flash中读取数据的公开、标准接口,可使用系统软件查询到Flash器件的各种参数(包括:Flash的制造商ID和设备ID,Flash的大小等等).
