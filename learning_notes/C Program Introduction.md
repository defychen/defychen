...
操作数各位右移,移出的低位丢弃,高位如下处理:
	...
	1.对于无符号数和有符号中的整数,直接补0.
		...
		a = a >> 2; //因为a为无符号数,因此a右移之后的值为1(0b1).
	2.有符号中的负数(即最高位为1),分为:
...
**位运算实例---将一个整数(16-bit)按二进制位输出**
	...
	int main()
	{
		...
		for (i=1; i<=16; i++) {
			...
			/*
			最高bit为1,putchar输出字符'1';否则输出字符'0'.
			此处必须为'1'/'0',不能是"1"/"0";putchar接收一个int类型,char可以转成int,但是str不能转int.
			*/
			num = num << 1;
			if (i % 4 == 0)
				putchar(','); //输出一个','字符
...
1.错误实例---申请不到内存
	...
	/*
		实参调用形参时,都是实参的一份拷贝,所有拷贝的本身是不能被改变的.
		e.g.
			1.传变量:此时变量本身就是值,因此该值不能被改变;
			2.传指针:此时指针本身就是地址,因此该地址不能被改变(即此处的malloc不能改变到实参的指针值).
				但是指针指向的内容(即地址中的内容是可以被改变的,也就是传指针可以改变值的原因).
			3.传引用:因为引用就是实参的别名,是同一个东西,因此可以改变值.
		此处传指针:本身为地址,	因此该地址不能被改变.仍未NULL.
	*/
...
2.修改---传指针的指针
	...
	/*
		此处传的是指针的指针(即地址的地址),该指针的指针不能被改变.但是指针的指针的内容(即*p--->地址),
		是可以被改变的.因此才能申请到内容.
	*/

3.函数形参传变量,指针,指针的指针比较

	1.传变量:调用形参时,为实参的一份变量的拷贝.如果在函数内修改形参变量并不能体现在实参变量上;
	2.传指针:调用形参时,为实参的一份指针(地址)的拷贝.在函数内修改形参指向的内容可以体现在实参上,
		但是实参的地址是改不动的(因为是一份拷贝);
...
**2.定义一个函数指针类型---常用**
	...
	test_func func_sum = NULL; //由函数指针类型定义一个函数指针func_sum
...
2.在模块内(e.g.文件内)但在函数体外:声明为静态的变量可以被模块内所有函数访问,但是不能被模块外其他函数访问;
...
例子:
	...
	int main()
	{
		int a[5] = {24,35,45,54,56};
		int (*p)[5] = &a; //给数组指针p赋值为&a.p和普通变量一样,只是该指针指向一个数组.
		printf("a->address:%x\n", &a);	//取a的地址
		printf("p->ptr:%x\n", p);		//p代表的即为a的地址,即与"&a"相等.
		printf("a->content:%x\n", a);	//此处a为a[0]的地址,虽然与"&a"相等,但是意义不同.
		printf("p->content:%x\n", *p);	//*p放的是a(即a[0]的地址).
		printf("a[2] value: %x\n", a[2]);
		printf("p index=2 value: %x\n", p[0][2]);
		/*
			p[0]:与*p/a是一样的.即为a[0]的地址.
				e.g.
					int b = 20;
					int *pb = &b; //pb就是b的地址.如果要打印出b的值可以:*pb或者pb[0]是一样的.
			p[0][2]:拿到数组中的第二个元素.
			p[0][3]:拿到数组中的第三个元素.
			p[1][3]--->不对,因为p[0]指向数组,p[1]没有得到具体的赋值.
		*/
	}
...
实例:
	...
	unsigned char **pTest = new unsigned char *[5]; 
	/*
