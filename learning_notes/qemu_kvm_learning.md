...
VMX root模式的下半部分是内核层.其中包含KVM驱动,KVM驱动以杂项(misc)设备驱动的方式存在于内核中.主要特点:

```
1.KVM通过"/dev/kvm"设备文件导出一系列的接口,Qemu等用户态程序可以通过这些接口来控制虚拟机的各个方面:
	e.g. CPU个数、内存布局、运行等.
2.KVM需要截获虚拟机产生的虚拟机退出(VM Exit)事件并进行处理.
```

### 1.2.3 各个组件虚拟化

Qemu+KVM联合起来共同完成虚拟机各个组件的虚拟化.
...
```
1.Qemu创建虚拟机CPU线程,在初始化的时候会设置好相应的虚拟CPU寄存器的值;
2.调用KVM接口,将虚拟机运行起来,在物理CPU上执行虚拟机的代码;
3.在运行过程中,KVM会截获虚拟机中的敏感指令.当虚拟机中的代码是敏感指令或满足了一定的退出条件时,CPU会从VMX non-root模式退出到
	KVM(VMX root模式的内核层),此时叫做VM Exit(类似用户态执行指令陷入内核态);
4.虚拟机的退出首先陷入到KVM中进行处理,如果KVM无法处理(e.g. 虚拟机写了设备的寄存器地址),KVM会将这个写操作分派到Qemu中进行处理,
	当KVM或Qemu处理好了VM Exit事件后,又可以将CPU置于VMX non-root模式运行虚拟机代码,此时叫做VM Entry;
5.总结就是:
	VMX non-root(虚拟机)--->VMX root kernel(KVM):通过VM Exit切换;
	VMX root kernel(KVM)---->VMX non-root(虚拟机):通过VM Entry切换.
	PS:VM Exit和VM Entry通过一个VMCS结构来保存.
```
...
```
1.Qemu在初始化的时候调用KVM的接口向KVM告知虚拟机所需的物理内存,即:
	Qemu通过mmap系统调用分配虚拟内存作为虚拟机的物理内存.
2.虚拟机内存转换方法:
	1.以前的做法:
		GVA--->GPA:虚拟机页表完成转换;
		GPA--->HVA(host virtual address):虚拟机的物理地址到宿主机的虚拟机地址;
		HVA--->HPA:宿主机完成.
	2.影子页表(软件实现):
		影子页表负责完成虚拟机虚拟地址--->宿主机物理地址的转换,纯软件实现并维护.
	3.EPT(Extended Page Table:扩展页表),现在x86 CPU都支持:
		EPT完成虚拟机物理地址到宿主机物理地址的转换.
		1.虚拟机第一次访问内存的时候陷入到KVM,KVM会建立起EPT页表;
		2.后续虚拟机访问虚拟机虚拟地址的流程为:
			GVA--->GPA:虚拟机页表完成转换;
			GPA--->HPA:EPT页表实现.
```
...
```
1.纯软件模拟:刚开始的Qemu只有纯软件模拟,虚拟机内核不做任何修改,每次对设备的寄存器读写都会陷入到KVM,进而到Qemu,Qemu再对这些
	请求进行处理并模拟硬件行为.
	--->纯软件模拟存在非常多的Qemu/KVM介入,效率不高;
2.virtio设备方案:virtio设备是一类特殊的设备,并没有对应的物理设备.需要虚拟机中运行的操作系统加载virtio驱动.virtio将Qemu变成
	了半虚拟化方案,本质上修改了虚拟机操作系统内核.
	--->virtio方案仍然不能满足一些高性能的场景.
3.设备直通方案:将物理硬件设备直接挂在虚拟机上,虚拟机直接与物理设备交互,减少I/O路径上Qemu/KVM的参与.
	--->与设备直通方案一起使用的又设备的硬件虚拟化支持技术SRIOV(Single Root I/O Virtualization)，SRIOV能将单个的物理硬
	件高效地虚拟出多个虚拟硬件,通过将SRIOV虚拟出来的硬件直通到虚拟机中,虚拟机能够非常高效的使用这些设备.
```
...
```
纯软件模拟--->virtio--->VFIO设备直通.
```

设备与总线的关系及Qemu的处理:

```
1.总线下面只能连接设备,设备必须连接到总线上.总线与总线之间、设备与设备之间不能直接连接;
2.Qemu模拟了ISA、PCI、USB等传统总线,也有虚拟化环境特有的virtio总线.
```
