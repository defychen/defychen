...
1.传统网络(基于Socket):需要3次拷贝操作:
	1.用户层到内核层的socket buffer的拷贝;
	2.内核层的协议栈操作--->从socket buffer中拷贝数据并经过TCP/IP协议栈放入Ring buffer;
	3.网卡通过DMA拷贝将Ring buffer中的数据搬移到网卡内部的缓冲区中.
...
# 2. 通信对比---传统以太网 v.s RDMA技术

## 2.1 传统以太网

典型的基于传统以太网的通信过程如下:

![](images/traditional_ethernet_communicating_flow.png)

### 2.1.1 收-发步骤

![](images/ethernet_send_recv_flow.png)

![](images/ethernet_send_recv_flow_1.png)

```
1.发送端操作流程
	1.发送端和接收端通过socket库提供的接口建立链接(就是在两个节点间建立了一条逻辑上的道路,数据可以沿这条道路从一端发送到另一端)
		并分别在内核内存中申请好发送和接收的buffer;
	2.发送端APP通过send/write将数据从user space写入到kernel space的socket buffer中(经历一次copy动作);
	3.内核态的socket buffer中的数据经过TCP/IP协议栈的一层层封装,最后被CPU拷贝到NIC driver能访问的ring buffer中(此时
		ring buffer存放的是可以被NIC硬件直接发送的已经封装好的数据报文)--->符合生产者消费者模型;
	4.CPU通过网卡驱动告知NIC硬件可以发送数据了,网卡硬件将通过DMA从ring buffer中拷贝封装好的数据报文到内部缓存中,然后将其发
	送到物理链路上;
2.接收端操作流程
	1.接收端网卡收到数据包后,执行DMA操作将数据报文拷贝到接收的ring buffer中;
	2.完成DMA操作后,NIC驱动会告知CPU(估计是通过中断的方式);
	3.CPU通过内核中的TCP/IP协议栈进行层层解析,取出有效数据.并拷贝到接收的socket buffer中(经历一次copy动作);
	3.接收端APP通过socket读取(recv/read)报文,此时CPU会将数据从kernel space的socket buffer中拷贝到user space.
```

### 2.1.2 瓶颈点

```
1.数据在user space与kernel space的socket buffer中的搬移需要CPU执行拷贝动作;
2.数据在socket buffer与ring buffer间的组包、解析需要CPU的参与,协议栈的处理会给CPU造成很大的负担,同时此处还需要CPU的拷贝.
```

## 2.2 RDMA

RDMA的处理可以分成2部分(控制通路和数据通路).

```
1.控制通路:准备RDMA通信所需的内存资源;
2.数据通路:指的是实际数据交互过程中的流程.
```

![](images/rdma_flow_1.png)

### 2.2.1 收-发步骤

```
1.发送端和接收端分别通过控制通路创建好通信所需要的内存资源(包括用户态资源及内核态资源);
2.在数据通路上,接收端APP通知硬件准备接收数据,告诉硬件将接收到的数据放在哪片内存中--->先要准备接收信息,否则会出现丢失现象???;
3.在数据通路上,发送端APP通知硬件发送数据,告诉硬件待发送数据位于哪片内存中;
4.发送端RDMA网卡从发送端内存中搬移数据,组装报文发送给对端;
...
```
1.数据收发绕过了内核并且数据交换过程并不需要CPU参与--->不需要CPU的拷贝;
2.报文的组装和解析是由RDMA硬件自己完成的--->不需要CPU进行协议栈的解析.
```
