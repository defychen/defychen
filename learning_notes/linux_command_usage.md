...
### 28.2 rm -rf出现"Device or resource busy"的解决方法

在执行rm -rf file出现"cannot remove .nfs... Device or resource busy"的解决方法:
...
/*
	-type d:表示查找类型为目录;
	grep .svn$:表示只查到文件夹的顶层,过滤掉.svn下面的目录;
	xargs rm -rf:执行删除命令
*/
```
...
1.删除所有.exe文件
	...
	/*
		-type f:表示查找类型为文件;
		-print:输出查找到的文件;
		-exec rm -rf {} \;--->必须是这种格式,后面有"\;",否则执行不成功
	*/
2.删除.svn目录
...
taskset命令,可用于将进程指定在某个CPU上工作(此处的进程就是线程).
...
2.获取进程状态(拿到进程对应的PID,比如此处pid为:23989)
...
3.查看进程运行在哪个CPU上
	...
	*/
	...
	taskset -pc pid(进程ID)
	/*
		显示为: pid 23989's current affinity list: 0-3
		表示该进程可运行在0-3号核心上.
	*/

4.将进程绑定到某个CPU核上

	1.taskset -pc 1 pid(进程ID)	//将进程绑定到Core #1上(即第二个CPU上)
	2.taskset -pc 0,2 pid // 将进程绑定到Core #0核Core #2上,进程可以在这两个Core上运行
		...
		*/
	3.taskset -p 7 pid
	// -p:以掩码的形式表现,显示进程可以在0x7(0b111)即可以在Core #0,#1,#2三个Core上运行
		taskset -p 11498
		/*
		显示为:
